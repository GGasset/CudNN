...
int main()
{
	...
	const size_t input_length = 27;
	const size_t output_length = 36;
	...
	size_t shape[shape_length]{ input_length, 32, 32, output_length };
	...
	for (size_t i = 1; i < shape_length; i++)
	{
		IConnections* connections = 0;
	        //connections = new DenseConnections(previous_layer_start, shape[i - 1], shape[i]);
		connections = new NeatConnections(previous_layer_start, shape[i - 1], shape[i]);
	...
	NN n = NN(layers, input_length, shape_length - 1);
	n.stateful = true;
	for (size_t i = 0; i < 50; i++)
	{
		...
		data_t* y = 0;
		//y = n.execute(X);
		n.supervised_train(t_count, X, Y_hat, true, CostFunctions::MSE, .5 / t_count, &y, true, 200000, 0);
		for (size_t t = 0; t < t_count; t++)
		{
